...
aad, plaintext = read_data_from_file("/mnt/d/Tesi/Ascon128/myAscon-128a/data.txt")

#-----------------STAMPA DI BEBUG-----------------
#print("🟦 Associated Data (AAD):")
#print(f"  Lunghezza: {len(aad)} byte")
#print("  Dati:     ", ' '.join(f"{byte:02x}" for byte in aad))

# Stampa Plaintext
#print("\n🟩 Plaintext:")
#print(f"  Lunghezza: {len(plaintext)} byte")
...
# Estrai il vero ciphertext (tutti i byte tranne il tag)
ct = ciphertext[:-16]

# Prendi solo i primi N byte, esattamente quanto il plaintext (senza stampare spazzatura)
ct_trimmed = ct[:len(plaintext)]

# Stampa
print("Ciphertext:", ct_trimmed.hex())
print("Tag:", tag.hex())
